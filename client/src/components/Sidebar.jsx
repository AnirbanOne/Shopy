...
console.log(import.meta.env.VITE_APP_BASE_URL);
  ...
  const handlePayment = async () => {
    ...
    const response = await axios.post(
      `${import.meta.env.VITE_APP_BASE_URL}/create-checkout-session`,
      {
        cart,
        total,
      }
    ...
    const results = await stripePromise.redirectToCheckout({
    ...
    >
      ...
      <div className="  flex flex-col gap-y-3 py-4 mt-4">
        <div className="flex w-full justify-between items-center">
          ...
          <div
            onClick={clearCart}
            className="cursor-pointer py-4 rounded-full bg-red-500 text-white w-12 h-12 flex justify-center items-center text-xl"
          >
            <FiTrash2 />
          </div>
        </div>
